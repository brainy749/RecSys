...
# Divided the dataset into training and testing . Above code is same for all the Recommendations 

# Working on Item-Item Collabrative filtering 
Products <- as.list(colnames(spreaddata))
pearson_correlation <- matrix(data = NA , nrow = ncol(spreaddata), ncol = ncol(spreaddata), dimnames = list(c(Products),c(Products)))
...
write.csv(Predictedratings,"Predictions_corr.csv")
...
sum(RMSE$SQUARE)/nrow(RMSE) # 0.4470321
...
correlation <- cor(Correlation$Ratings.x,Correlation$Ratings.y) # 0.500764  It is very Good
